...
function exportF(elem) {
  ...
  elem.setAttribute("download", "tipo_produccion.xls"); // Choose the file name
  return false;
}
</script>
...
<div class="container">
      <div class="row justify-content-left">
        <div class="col-md-3">
			...
			<a href="{{url('/ingreso_tipo_produccion/create')}}" class="btn btn-success" >Agregar Tipo Producción</a>
        ...
        <div class="col-md-12">
		    ...
		    <div style="overflow-x:auto;">      
		     <table class="table table-light table-striped table-bordered" id="example"  style="width:100%">
			    ...
			    <tbody>
				 @foreach($ing_tipo_produccion as $producto)
				  <tr>
					 <td>{{$loop->iteration}}</td>
				   <td>{{$producto->nombre_tipo_tacho}}</td>
						
					<td>
					 	<div class="btn-group">
		            	<a href="{{url('/ingreso_tipo_produccion/'.$producto->id.'/edit')}}" class="btn btn-warning"><i class="fa fa-pencil" ></i></a>
					  		

					 	 <form method="post" action="{{url('ingreso_tipo_produccion/'.$producto->id)}}">
					 		{{csrf_field()}}
					 		{{method_field('DELETE')}}
