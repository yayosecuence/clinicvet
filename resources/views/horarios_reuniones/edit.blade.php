...
<div class="container">
    ...
    <div class="row justify-content-center">
        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">Formulario para Modificar horarios de reuniones</div>
               <div class="col-md-12">
               	
            		<form action="{{url('/horarios_reuniones/'.$reuniones->id)}}" method="post" enctype="multipart/form-data">
{{csrf_field()}}
{{method_field('PATCH')}}
<label for="hora" class="col-md-4 col-form-label text-md-left" >{{'Hora de Reunion:'}}</label>
<input type="date" class="form-control" name="hora" id="hora" value="{{$reuniones->hora}}">


<label for="dia" class="col-md-4 col-form-label text-md-left" >{{'Fecha de Reunion:'}}</label>
<input type="time" class="form-control" name="dia" id="dia" value="{{$reuniones->dia}}">

<label for="motivo" class="col-md-4 col-form-label text-md-left" >{{'Motivo:'}}</label>
<input type="text" class="form-control" name="motivo" id="motivo" value="{{$reuniones->motivo}}">
