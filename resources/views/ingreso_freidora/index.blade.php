...
function exportF(elem) {
  ...
  elem.setAttribute("download", "numero_de_freidoras.xls"); // Choose the file name
  return false;
}
</script>
...
<div class="container">
      <div class="row justify-content-left">
        <div class="col-md-3">
			...
			<a href="{{url('/ingreso_freidora/create')}}" class="btn btn-success" >Agregar nueva Freidora</a>
        ...
        <div class="col-md-12">
		    ...
		    <div style="overflow-x:auto;">      
		     <table class="table table-light table-striped table-bordered" id="example"  style="width:100%">
			    ...
			    <tbody>
				 @foreach($ing_freidora as $producto)
				  <tr>
					 <td>{{$loop->iteration}}</td>
				   <td>{{$producto->numero_freidora}}</td>
						
					<td>
					 	<div class="btn-group">
		            	<a href="{{url('/ingreso_freidora/'.$producto->id.'/edit')}}" class="btn btn-warning"><i class="fa fa-pencil" ></i></a>
					  		

					 	 <form method="post" action="{{url('ingreso_freidora/'.$producto->id)}}">
					 		{{csrf_field()}}
					 		{{method_field('DELETE')}}
